...
class NonImageElement extends Component{
    ...
    submitModalHandler = async (evt) =>{
        ...
        let data = {
            buyer_id,
            buyer_colValue : this.state.newValue,
            buyer_colName : this.state.element
        }
        await axios({
            method: 'post',
            url: "http://ec2-54-147-235-117.compute-1.amazonaws.com:3001/updateBuyerProfile",
...
class ProfileImage extends Component{
    ...
    submitModalHandler = async (evt) =>{
        ...
        await axios({
            method: 'post',
            url: "http://ec2-54-147-235-117.compute-1.amazonaws.com:3001/updateBuyerProfileImage",
            // data: {"jsonData" : JSON.stringify(data)},        
            data,
            config: { headers: { 'Content-Type': 'multipart/form-data' } }
        })
...
export class BuyerProfile extends Component{
    ...
    componentDidMount=async()=>{
        ...
        await axios({
            method: 'post',
            url: "http://ec2-54-147-235-117.compute-1.amazonaws.com:3001/getBuyerProfile",
            // data: {"jsonData" : JSON.stringify(data)},        
            ...
            .then((responseData) => {
                ...
                if(responseData.status){
                    this.setState({
                        isRendered : true,
                        buyerProfile : responseData.message
                    });
                } else {
                    //alert(responseData.message);
                }
    ...
    render(){
        let renderProfileElements=()=>{
            let allEles= [];
            for(let element in  this.state.buyerProfile){
                let value = this.state.buyerProfile[element];
                //allCards.push(this.renderCardComponent(element, value));
                if(element != "buyer_profileImage"){
                    allEles.push(<NonImageElement key={element} element={element} value={value}/>);
                }
            }
            allEles.push( <ProfileImage key="buyer_profileImage" buyer_profileImage={this.state.buyerProfile.buyer_profileImage}/>);
            return allEles;
        }
        let renderProfile = () => {
